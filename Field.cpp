...
//        this->getCell(start_pos).playerStepped();
        setPlayerPosition(start_pos, false);
    }
}

Field::Field(int width, int height, std::vector<std::vector<Cell>> &&cells)
        : _width(width), _height(height), _matrix(std::move(cells)), _player_pos({}) {}
...
Field::Field(const Field &other) : _width(other._width), _height(other._height), _matrix(other._matrix),
                                   _player_pos(other._player_pos) {}

Field::Field(Field &&other) noexcept
        : _width(other._width), _height(other._height), _matrix(std::move(other._matrix)),
          _player_pos(other._player_pos) {
    other._height = 0;
    other._width = 0;
    other._matrix = {};
...
const Cell &Field::getCell(Coords coordinate) const {
...
void Field::setPlayerPosition(Coords new_pos, bool previous) {
